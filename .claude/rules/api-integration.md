...
- **Architecture** (v0.10.5): agent plumbing split into four files — `AgentConversationLoop` (SSE loop, tool-use state machine, retry/wrap-up), `AgentToolExecutor` (7 tool implementations, ID-addressed resolution), `AgentPromptBuilder` (cached + fresh system prompt, wardrobe alias index), `OutfitMatcher` (alias + fuzzy resolution). `AgentViewModel` is the observable facade that conforms to `AgentToolHost` + `AgentLoopHost`.
