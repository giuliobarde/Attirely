...
- `ClothingItem.aiOriginalValues: String?` — JSON snapshot of original AI-detected field values; stored at scan time for diffing when user edits
- `Outfit.displayName` computed property falls back: `name` → `occasion` → formatted date
- `Outfit.lastSuggestedBySiriAt: Date?` — analytics-only timestamp; does **not** influence outfit selection
...
- `StyleSummary.behavioralNotes: String?` is JSON-encoded `[AgentObservation]`; use `behavioralNotesDecoded` computed property. `activeObservations` filters for active + non-stale, sorted by recency
- `UserProfile` location override: `isLocationOverrideEnabled: Bool`, `locationOverrideName: String?`, `locationOverrideLat: Double?`, `locationOverrideLon: Double?` — custom city for weather; overrides CoreLocation when enabled
...
- `StyleAnalysisDTO.styleModes` defaults to empty array if null; each entry is a `StyleModeDTO` (name, description, colorPalette, formality)
...
- `ScanSession.swift` is a lightweight SwiftData model grouping items from a single scan (id, imagePath, date, items relationship) — no business logic
